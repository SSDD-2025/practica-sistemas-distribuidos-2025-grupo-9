...
2. Una vez conectados desde la maquina virtual sidi09-2 ejecutamos la bbdd:
   ````shell
   docker run --rm -e MYSQL_ROOT_PASSWORD=admin -e MYSQL_DATABASE=clubtenis -v mysql_data:/var/lib/MySQL -p 3306:3306 -d mysql:9.2
   ````
3. Una vez desplegada la bbdd desde sidi09-1 ejecutamos la aplicación:
   ````shell
   docker run -p 8443:8443 -e SPRING_DATASOURCE_URL=jdbc:mysql://192.168.110.194:3306/clubtenis -e SPRING_DATASOURCE_USERNAME=root -e SPRING_DATASOURCE_PASSWORD=admin rodrilb/clubtenis:1.0.0
   ````
